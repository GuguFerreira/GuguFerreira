🛡️ Portfólio de Cibersegurança.

Bem-vindo ao meu repositório de estudos e projetos em Cibersegurança.
Aqui você encontrará laboratórios práticos, análises de vulnerabilidades, automações e estudos voltados para segurança ofensiva e defensiva.

🚀 Sobre mim
Sou um entusiasta de cibersegurança focado em:

🔍 Análise de incidentes
🛠️ Pentest (Testes de invasão)
🧠 Segurança defensiva (Blue Team)
⚙️ Automação com scripts

Atualmente estou estudando e desenvolvendo habilidades práticas para atuar como Analista de Segurança Júnior.

🧪 Laboratórios e Projetos
🔴 Red Team (Ofensivo)
🔓 Exploração de vulnerabilidades (OWASP Top 10)
💣 Simulações de ataques controlados
🐍 Scripts para automação de testes
🕵️ Enumeração e reconhecimento
🔵 Blue Team (Defensivo)
📊 Análise de logs
🚨 Detecção de intrusões
🔍 Investigação de incidentes
🛡️ Hardening de sistemas

⚙️ Ferramentas utilizadas
Kali Linux,
Wireshark,
Nmap,
Burp Suite,
Metasploit,
Python / Bash,
C,
C++,
Rust,
C#

📬 Contato:
Linkedin: https://www.linkedin.com/in/gustavo-ferreira-cordeiro-543048369/

Email: azagustavo98@gmail.com

